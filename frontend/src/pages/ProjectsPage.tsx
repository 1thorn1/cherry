    ...
    <div className="mx-auto max-w-5xl px-5 py-6 lg:px-8 lg:py-10">
      ...
      {overview && overview.focus && (
        <div className="mb-8 rounded-lg border border-neutral-200 p-4">
          ...
          {overview.focus.milestones.length === 0 ? (
            <p className="py-4 text-center text-xs text-neutral-400">마일스톤이 없어요</p>
          ) : (
            <div className="overflow-x-auto pt-5">
              <div className="relative" style={{ minWidth: overview.focus.milestones.length * 16 }}>
                <div
                  className="absolute -top-5 flex flex-col items-center text-[var(--cherry)]"
                  style={{
                    left: `${(overview.focus.cart_index / overview.focus.milestones.length) * 100}%`,
                ...
                <div className="flex gap-0.5">
                  {overview.focus.milestones.map((m) => (
                    <div
                      key={m.seq}
                      className="h-2 flex-1 rounded-sm"
                      style={m.completed
                        ? { background: 'var(--cherry)' }
                        : { border: '1.5px dashed #D8D5CC' }}
                      title={m.title}
                    />
                  ))}
                </div>
