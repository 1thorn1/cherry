...
| 2026-09-22 | `backend/src/main/java/com/cherry/project/ProjectService.java` (`generateExamMilestones`, V4 프로젝트 기능에서 도입) | 시험형(`EXAM`) 프로젝트를 `total_units` 없이 생성하면 `Project.getTotalUnits()`가 `null`을 반환해 언박싱 시 `NullPointerException` 발생 (500 에러). `ProjectCreateRequest`에 타입별 필수값 검증이 없음. 프로젝트 목록 화면(A-6-3) 작업 중 테스트 데이터를 만들다 발견 | 미수정 — 현재 작업(A-6-3) 범위 밖이라 기록만 남김 |
