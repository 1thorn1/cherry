...
### 원칙 5 — 월 그리드를 할 일 체크 표면으로 만들지 않는다
하나라도 만들면 결국 거기로 들어가게 되고, 애초에 벗어나려던 경험이 돌아온다.
시간축은 **주간 시간표**와 **프로젝트 레인**으로 표현하고, 체크는 오늘 화면에서만 일어난다.

*(2026-09-21 수정)* 월 단위로 "이번 달에 뭐가 있었는지" 한눈에 보고 싶다는 요구는 실제로 있었다. 그래서 **날짜 칸에 완료 개수 배지만 보여주고, 누르면 그 주 시간표로 이동하는** 읽기 전용 월 그리드는 예외로 허용한다(A-6-2). 금지하는 것은 여전히 유효하다 — **그 그리드 자체가 제목을 늘어놓고 체크박스를 누르는 화면이 되는 것.**
...
- **캘린더** — 주간 시간표 + 월간 요약 그리드(읽기 전용, A-6-2)
...
**월간 뷰 (2026-09-21 추가, 원칙5 수정)**

주간 시간표만으로는 "이번 달에 뭐가 있었는지"가 한눈에 안 보인다는 실사용 피드백으로 월 그리드를 추가했다. 다만 원칙5가 막으려던 것 — **그리드 자체가 할 일을 체크하는 표면이 되는 것** — 은 그대로 막는다.

- 날짜 칸에는 **날짜 숫자 + 그 날 완료 개수 배지**만 표시. 할 일 제목·체크박스는 없음
- 칸을 누르면 **그 주의 주간 시간표로 이동**할 뿐, 그 자리에서 체크하거나 상세를 보는 기능은 없음
- 즉 월간 뷰는 **탐색/개관용 진입점**이지 계획을 세우거나 체크하는 화면이 아니다. 실제 체크는 여전히 오늘 화면에서만 일어난다

이 조건을 벗어나 그리드 안에서 직접 체크하거나 제목을 늘어놓는 방향으로 가면 원칙5가 경고한 상태로 되돌아가는 것이므로 다시 논의한다.
...
### 만들지 않는 것
- 할 일 제목·체크박스가 있는 월 그리드 캘린더 (완료 개수 배지만 보여주는 읽기 전용 월간 뷰는 예외 — 원칙5, A-6-2)
...
스펙 결함이 아니라 **구현 버그**. 작업 중 발견할 때마다 여기 추가한다.
...
| 2026-09-22 | `backend/src/main/java/com/cherry/project/ProjectService.java` (`generateExamMilestones`, V4 프로젝트 기능에서 도입) | 시험형(`EXAM`) 프로젝트를 `total_units` 없이 생성하면 `Project.getTotalUnits()`가 `null`을 반환해 언박싱 시 `NullPointerException` 발생 (500 에러). `ProjectCreateRequest`에 타입별 필수값 검증이 없음. 프로젝트 목록 화면(A-6-3) 작업 중 테스트 데이터를 만들다 발견 | 수정 완료 — B-11 정리 작업(2026-09-23) 중 `ProjectService.createProject()`에 `validateTypeFields()` 추가. `PROGRESS`는 `total_units`, `EXAM`은 `total_units`·`exam_date`를 모두 요구하고, 누락 시 500 대신 400(`INVALID_REQUEST`) 반환 |
| 2026-09-22 | `frontend/src/pages/TodayPage.tsx` (미완료 이월 A-11 작업 중 직접 작성한 헬퍼) | `tomorrowDate()`가 `new Date(...).setDate()` 후 `.toISOString()`으로 날짜만 뽑았는데, `toISOString()`은 UTC 기준이라 UTC+9(KST)에서는 자정 근처 계산이 하루 밀려 "내일" 버튼을 눌러도 오늘 날짜 그대로 저장됨. Playwright로 미루기 동작을 검증하다가 네트워크 요청 페이로드에서 발견 | 수정 완료 — 같은 작업(2026-09-22) 중 `Date.UTC()`로만 날짜를 계산하도록 `addDays()` 헬퍼로 교체 |
| 2026-09-23 | `backend/src/main/resources/db/migration/V13__daily_stat_point_basis.sql` (반복 어뷰징 방어 A-6-7 작업 중 직접 작성한 마이그레이션) | `daily_stat`에 포인트 계산 전용 `point_basis` 컬럼을 새로 추가하면서 기존 행을 0으로만 채우고 지난 `points_earned`와 맞춰 채워두지 않음. 이 상태로 두면 오늘 이미 완료가 있던 날 다음 완료 시 `point_basis=0`에서 새로 계산해 기존보다 훨씬 낮은 값이 나와 포인트가 갑자기 깎여 보임 (실사용자 포인트 손실로 이어질 뻔함). 마이그레이션 적용 직후 daily_stat 값을 직접 확인하다가 발견 | 수정 완료 — 같은 작업(2026-09-23) 중 `V14__backfill_point_basis.sql`로 `point_basis = completed_count * 2`(기존 방식과 동일한 계산) 백필 |
| 2026-09-23 | `backend/src/main/java/com/cherry/park/ParkService.java` (`awardForMilestoneCompletion`, V6 공원 기능에서 도입) | 다음 놀이기구 슬롯 번호를 `countByUserId(userId) + 1`로 계산. 과거에 슬롯 행이 삭제돼 번호에 구멍이 생기면(예: 1,2,3,6,7,8 — 행 개수는 6인데 실제 쓰인 최댓값은 8) `count+1`이 이미 쓰인 번호와 겹쳐 `park_slot.uk_slot`(user_id, slot_index) 유니크 제약 위반으로 완료 트랜잭션 전체가 롤백됨. `TaskService.complete()`가 같은 트랜잭션이라 마일스톤·태스크 완료까지 통째로 취소돼 "완료 처리가 안 된다"로 보임. 자유형 프로젝트 마일스톤 완료가 매번 실패한다는 리포트를 받고 백엔드 로그에서 `DataIntegrityViolationException`을 보고 발견 | 수정 완료 — 같은 작업(2026-09-23) 중 `findTopByUserIdOrderBySlotIndexDesc()`로 실제 최댓값 기준으로 다음 번호를 잡도록 변경 |
| 2026-09-23 | `frontend/src/pages/ProjectDetailPage.tsx` (마일스톤 완료 UX 보완 작업 중 도입) | 어떤 요청이든 하나라도 실패하면 `error` state가 페이지 전체를 대체해 삭제·공유·완료 버튼까지 통째로 사라짐. 위 `park_slot` 버그로 완료 요청이 실패할 때마다 화면이 빨간 에러 문구 한 줄로 굳어버려 "완료 처리가 안 된다"는 리포트를 더 헷갈리게 만듦 | 수정 완료 — 같은 작업(2026-09-23) 중 에러를 페이지 안 배너로만 표시하도록 변경, 나머지 UI는 그대로 유지 |
...
| 2026-09-25 | `frontend/src/pages/TodayPage.tsx` (캘린더 클릭→투두 이동 기능, 이번 세션 `feat/calendar-click-to-today` 작업에서 도입) | 캘린더에서 특정 날짜를 눌러 `/?date=2026-09-24`로 들어온 뒤에는, "오늘로" 버튼이나 상단 날짜 선택기로 다른 날짜(오늘 포함)를 골라도 화면이 그 날짜로 안 바뀌었음. `viewDate`를 바꿔도 URL의 `date` 파라미터는 그대로라, `searchParams`를 지켜보는 다른 `useEffect`가 "URL엔 아직 옛날 날짜가 남아있다"고 판단해 방금 바꾼 값을 곧바로 되돌렸음(같은 렌더 루프 안에서 자기 자신을 무효화). 자정에 자동으로 "오늘"로 넘어가는 로직도 같은 구조라 한 번 지나면 다신 자동으로 안 넘어가는 문제도 같이 있었음. "홈 화면이 오늘이 아니라 24일로 고정되어 있고 달력에서 수정이 안 되네" 리포트로 발견 | 수정 완료 — 같은 작업(2026-09-25) 중 날짜를 바꿀 때 `viewDate` state와 URL의 `date` 파라미터를 항상 같이 갱신(또는 제거)하는 `goToDate()` 헬퍼로 통일. 자정 자동 전환 로직도 새 "오늘"로 넘어갈 때 URL의 옛 파라미터를 같이 지우도록 수정 |
| 2026-09-25 | `frontend/src/pages/CalendarPage.tsx` (주간 캘린더, v1 캘린더 기능부터 있던 구조) | 완료된 일정을 표시할 때 "종일" 줄은 `line-through`(취소선)를 넣지만, 시간이 있는 예정 블록은 배경·글자색만 회색으로 바꾸고 취소선은 안 넣었음. 그래서 같은 날 완료된 항목이 종일 항목이면 줄이 그어지고 시간 있는 항목이면 안 그어져 "왜 두 개는 있고 두 개는 없냐"는 리포트로 발견 — 실제로 9/22에 완료된 4건 중 종일 2건은 취소선, 시간 있는 2건은 안 그어져 있었음. 1차 수정("예정" 탭만) 뒤에도 "여전히 2개밖에 없다"는 재리포트가 와서 확인해보니, "실제" 탭의 시간 있는 완료 블록에도 똑같이 취소선이 빠져 있었음(이 탭은 전부 완료된 항목이라 "당연히 다 완료"라 굳이 취소선이 필요 없을 거라 생각했는데, 종일 줄은 탭 구분 없이 항상 취소선이 있어서 실제 탭에서도 종일 vs 시간 있는 항목 간 불일치가 그대로 남아 있었음) | 수정 완료 — 같은 작업(2026-09-25) 중 "예정" 탭에 이어 "실제" 탭의 시간 있는 블록에도 `line-through` 추가, 두 탭·종일 줄 모두 동일하게 표시. 그런데도 2차 재리포트가 또 와서 Playwright로 직접 스크린샷을 찍어 확인해보니, `line-through`는 실제로 적용돼 있었지만(computed style로 확인) 텍스트 색과 취소선 색이 같은 옅은 회색(`#888780`)이라 10px 작은 글자에서 선이 글자에 묻혀 거의 안 보였던 것 — 진짜 원인은 로직이 아니라 대비 부족이었음 | 수정 완료 — `decoration-2`(두꺼운 선)와 더 짙은 `decoration-neutral-500`을 얹어 대비를 높임. Playwright 스크린샷으로 실제로 눈에 띄게 보이는 것까지 확인 |
| 2026-09-25 | `frontend/src/pages/CalendarPage.tsx` ("종일" 줄, 위 항목과 같은 리포트에서 같이 요청됨) | 완료된 종일 항목이 취소선과 함께 계속 쌓이는 구조라, 완료가 많은 날은 줄이 계속 길어져 지저분해 보임 ("이거 많아지면 더러우니까 접었다 폈다 할 수 있게도 해줘" 요청) | 반영 완료 — 같은 작업(2026-09-25) 중 미완료 종일 항목은 항상 보이고, 완료된 항목은 "완료 N개 ▾" 토글 뒤로 기본 접어서 날짜별로 펼치고 접을 수 있게 함 |
| 2026-09-25 | `frontend/src/pages/CalendarPage.tsx` ("완료 N개" 토글, 바로 위 항목에서 도입) | "완료 N개" 토글이 종일 항목만 세고 있어서, 시간 있는 완료 항목은 시간표에 그대로 남아 있었음. 화요일처럼 종일 2건 + 시간 있는 2건이 완료됐으면 토글엔 "완료 2개"만 뜨고 나머지 2개는 시간표에 별도로 떠 있어 총 개수가 안 맞아 보임 — "완료 2개라고 되어있는데 4개가 다 나오게 해줘" 리포트로 발견 | 수정 완료 — 같은 작업(2026-09-25) 중 "예정" 탭에서는 시간 있는 완료 항목도 시간표에서 빼서 같은 토글에 합치도록 변경(토글이 그날 완료 총합을 정확히 셈). "실제" 탭은 시간표가 "언제 했는지" 보여주는 화면이라 그대로 두고 종일 줄엔 종일 항목만 유지 — Playwright로 예정 탭에 "완료 4개"로 뜨고 펼치면 4건 다 보이는 것, 실제 탭은 기존대로인 것 확인. **→ 아래 항목에서 이 토글 구조 자체를 롤백함** |
| 2026-09-25 | (버그 아님 — 설계 되돌림) `frontend/src/pages/CalendarPage.tsx`, 바로 위 두 항목("완료 N개" 토글 도입·수정) | 토글로 완료 항목을 접어서 보여주는 구조를 두 차례 다듬었는데도 계속 "여전히 2개로 뜬다"는 리포트가 반복됐음. 확인해보니 "실제" 탭에서는 토글이 의도적으로 종일 항목만 세도록 설계했던 게 오히려 오늘 화면(TodayPage)의 시간표(Timetable.tsx)가 예정/실제 구분 없이 완료 항목을 토글 없이 제자리에 회색+취소선으로만 보여주는 기존 패턴과 달라서, 사용자가 "투두 예정 - 주간캘린더 예정, 투두 실제 - 주간캘린더 실제로 똑같이 동작해야 하는 거 아니냐"고 되물음. 토글 자체를 없애기로 결정 | 롤백 완료 — 같은 작업(2026-09-25) 중 "완료 N개" 토글과 관련 state(`expandedAllDay`)를 전부 제거하고, 종일·시간 있는 완료 항목 모두 예정/실제 탭 구분 없이 제자리에 회색 배경 + 두꺼운 취소선(`decoration-2 decoration-neutral-500`, 가독성 수정은 유지)으로 표시하는 원래 방식으로 되돌림 — TodayPage의 Timetable과 같은 패턴 |

---

## B-12. 구현 갭 (스펙 대비 미구현)

원래 12개 항목 감사가 끝난 뒤(2026-09-23) 스펙 전체를 다시 코드와 대조해 발견한 항목들. B-11과 달리 **버그가 아니라 스펙에 있는데 아직 손대지 않은 기능**이다. 우선순위 순.
...
**참고 (하드 갭 아님, 설계 판단 필요):** 마일스톤 칩을 다시 눌러 완료를 취소해도(`TaskService.uncompleteMilestoneNow`) 이미 지급된 포인트·인구·공원 슬롯은 그대로 둔다 — 일반 태스크 체크 해제(`Task.uncomplete()`)와 같은 원칙을 그대로 따른 것. 2026-09-24에 사용자가 이 "롤백이 부실하다"는 걸 직접 지적함 — 공원 시스템(포인트·인구·슬롯 지급/회수 규칙)을 제대로 설계할 때 완료 취소의 회수 로직도 같이 다시 잡을 것. 그때까진 의도적 현행 유지.
