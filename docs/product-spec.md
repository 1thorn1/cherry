...
| 2026-09-25 | `frontend/src/pages/CalendarPage.tsx` (주간 캘린더, v1 캘린더 기능부터 있던 구조) | 완료된 일정을 표시할 때 "종일" 줄은 `line-through`(취소선)를 넣지만, 시간이 있는 예정 블록은 배경·글자색만 회색으로 바꾸고 취소선은 안 넣었음. 그래서 같은 날 완료된 항목이 종일 항목이면 줄이 그어지고 시간 있는 항목이면 안 그어져 "왜 두 개는 있고 두 개는 없냐"는 리포트로 발견 — 실제로 9/22에 완료된 4건 중 종일 2건은 취소선, 시간 있는 2건은 안 그어져 있었음 | 수정 완료 — 같은 작업(2026-09-25) 중 "예정" 탭의 시간 있는 블록에도 완료 시 `line-through` 추가, 종일 줄과 같은 표시로 통일 |
